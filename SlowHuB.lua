...
function sameTeam(plr)
    ...
    if myChar and theirChar then
        ...
        if myHrp and theirHrp then
            pcall(function()
                if myHrp.BrickColor == theirHrp.BrickColor then return true end
            end)
        end
    end
        ...
        if myTorso and theirTorso then
            pcall(function()
                ...
                if mc and tc then
                    ...
                    if diff < 0.05 then return true end
                end
            end)
        end
    end
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        if isValidTarget(plr, Config.Aimbot.TeamCheck) then
            ...
            if char and hum and hrp 
                ...
                and hum:GetState() ~= Enum.HumanoidStateType.Physics then
                
                local vivo = true
                if hum.Health <= 0 or hum.Health / math.max(hum.MaxHealth, 1) < 0.05 then
                    vivo = false
                end
                if vivo then
                    local deadAttr = char:GetAttribute("Dead") 
                        or char:GetAttribute("IsDead") 
                        or plr:GetAttribute("Dead")
                    if deadAttr == true then vivo = false end
                end
                if vivo then
                    for _, obj in ipairs(char:GetChildren()) do
                        local n = string.lower(obj.Name)
                        if n:find("ragdoll") or n:find("dead") or n:find("corpse") then
                            vivo = false
                            break
                        end
                    end
...
-- NOCLIP (melhorado — checa Character e CharacterAdded)
noclipConn = nil
noclipCharConn = nil
...
-- SPEED (melhorado — sem lag mobile, checa CharacterAdded)
speedConn = nil
speedCharConn = nil
...
-- FLY (melhorado — suave, sem travar)
flyConn = nil
flyBodyVel = nil
flyBodyGyro = nil
flyCharConn = nil
...
-- ANTI-FLING (melhorado — só ativa em velocidade absurda)
antiFlingConn = nil
...
-- TP SAVE/GOTO (melhorado — sem bugar)
savedPositions = {}
